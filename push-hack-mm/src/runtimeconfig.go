...
// values watchMMPort/watchHWParams actually act on, and the on-screen I/O
...
// process restart needed — and iopage.go persists it to mm-config.json
// right after, so the choice survives a restart too. Adapted from
// push-hack-xenia/src/runtimeconfig.go, plus baseChannel.
...
// On/Off must arrive on to trigger the selected track, or -1 for omni.
...
// getBaseChannel/setBaseChannel: see persistedConfig.BaseChannel's doc.
func (s *sharedConfig) getBaseChannel() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.baseChannel
}

func (s *sharedConfig) setBaseChannel(ch int) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.baseChannel = ch
}
